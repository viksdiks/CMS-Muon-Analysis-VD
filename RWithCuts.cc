...
void RWithCuts(){
    ...
    TH1F *hMuon_pfRelIso04_all = new TH1F("hMuon_pfRelIso04_all", "PF relIso04;Iso;Events", 100, 0, 0.5);
    ...
    TH2F *hMjj_vs_Mmumu = new TH2F("hMjj_vs_Mmumu", "Dijet Mass vs Dimuon Mass; M_{#mu#mu} [GeV]; M_{jj} [GeV]", 100, 0, 150, 100, 0, 300); // <-- ДОБАВЕНО ТУК
    ...
    for (int i = 0; i < eventsToProcess; i++) {
        ...
        double m_jj = -1.0; // <-- ИНИЦИАЛИЗИРАНА ПРОМЕНЛИВА
        ...
        if (hasDiJet) {
            ...
            // ДОБАВЕНО ТУК: Пълним новата 2D хистограма
...
std::cout << "   -> REQUIRE: cos(deltaPhi) <= -0.8 (muons are back-to-back)\n";
std::cout << "   -> REJECT: cos(deltaPhi) > -0.8\n";
...
std::cout << "======================================================================\n";
    ...
    hMjj_vs_Mmumu->Write(); // <-- ДОБАВЕНО ТУК ЗА ЗАПАЗВАНЕ
