...
void WithoutCutsFA(){
    ...
    TFile *fout = new TFile("MonteCarloJets.root", "RECREATE");
    ...
    t1->SetBranchStatus("GenPart_statusFlags", 1); t1->SetBranchAddress("GenPart_statusFlags", GenPart_statusFlags);
    ...
    for (int i = 0; i < eventsToProcess; i++) {
        ...
        for (UInt_t gp = 0; gp < nGenPart; gp++) {
            Int_t gpflags = GenPart_statusFlags[gp];
            bool isLastCopy = gpflags & (1 << 13);
            bool isFromHardProcess = gpflags & (1 << 8);

            if (GenPart_pdgId[gp] == 23 && isLastCopy && isFromHardProcess) {
                genZ_pt = GenPart_pt[gp];
                zVec.SetPtEtaPhiM(GenPart_pt[gp], GenPart_eta[gp], GenPart_phi[gp], GenPart_mass[gp]);
                genZ_pz = zVec.Pz();
                
                if (i < 10) {
                    std::cout << "--> Събитие " << i << ": Намерен Z бозон с pT = " << genZ_pt << " (isLastCopy)" << std::endl;
                }
                break;
            }
        }
        ...
        if (hasDiJet) {
            hMjj_vs_Mmumu->Fill(mass_ll, mjj);
        }
...
void WithCuts(){
    ...
    TFile *fout = new TFile("Alabalapartakala2.root", "RECREATE");
    ...
    TH2F *hnGenJet_vs_GenZPz = new TH2F("hnGenJet_vs_GenZPz", "hnGenJet_vs_GenZPz", 100, -500, 500, 20, -0.5, 19.5);
    TH2F *hnGenJet25_vs_GenZPz = new TH2F("hnGenJet25_vs_GenZPz", "hnGenJet25_vs_GenZPz", 100, -500, 500, 20, -0.5, 19.5);
    TH2F *hCosDeltaPhiGenMu_vs_CosDeltaPhiJet = new TH2F("hCosDeltaPhiGenMu_vs_CosDeltaPhiJet", "hCosDeltaPhiGenMu_vs_CosDeltaPhiJet", 50, -1.1, 1.1, 50, -1.1, 1.1);
    TH2F *hGenZPt_vs_DiJetPt = new TH2F("hGenZPt_vs_DiJetPt", "hGenZPt_vs_DiJetPt", 100, 0, 300, 100, 0, 300);
    ...
    long totalEvents = 0;
    long passTwoMuons = 0;
    long passKinematics = 0;
    long passOppositeCharge = 0;
    long passTopology = 0;
